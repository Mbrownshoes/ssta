...
d3.timeDay.offset(d3.utcDay(new Date("2021-01-26")), -1)
)});
...
Inputs.radio(["anomaly", "Heat wave category"], {
  ...
  main.variable(observer("viewof datesToPlot")).define("viewof datesToPlot", ["colorView","mutable firstLoad","Inputs","datesCopy","limits"], function(colorView,$0,Inputs,datesCopy,limits)
{
  colorView;
  if ($0.value === 0) {
    $0.value = 1;
...
0
  ...
  main.variable(observer("viewof time1")).define("viewof time1", ["datesToPlot","md","textcolor","Scrubber"], function(datesToPlot,md,textcolor,Scrubber)
{
  // const button = document.querySelector('[name="b"]');
  // datesToPlot === null ? (button.disabled = true) : (button.disabled = true);

  if (datesToPlot === null) {
    return md`${textcolor("Click the button to animate", "black")}`;
    //   Scrubber([], {
    //   delay: 250,
    //   autoplay: false,
    //   loop: false,
...
function (
  ...
  let b = svg.append("g").call(brush).attr("fill-opacity", 1);
