...
import define5 from "./921d584bd92a9817@323.js";
...
d3.utcDay(new Date("2022-12-26"))
)});
  main.variable(observer("today")).define("today", ["d3"], function(d3){return(
d3.timeDay.offset(d3.utcDay(new Date("2021-12-26")), -1)
)});
...
limits
  ...
  main.variable(observer("original")).define("original", ["d3","start","today"], function(d3,start,today)
{
  const original = [
    d3.isoParse(start),
    d3.isoParse(d3.timeDay.offset(today, -1))
...
datesToPlot
  ...
  main.variable(observer()).define(["datesCopy","limits"], function(datesCopy,limits){return(
datesCopy.filter((d) => d >= limits[0] && d <= limits[1])
)});
  main.variable(observer("viewof datesToPlot")).define("viewof datesToPlot", ["mutable thing","Inputs","datesCopy","limits"], function($0,Inputs,datesCopy,limits)
{
  if ($0.value === 0) {
    $0.value = 1;
    return Inputs.button("Submit to load", {
      value: datesCopy.filter((d) => d >= limits[0] && d <= limits[1]),
      reduce: () => {
  ...
  main.variable(observer("datesToPlot")).define("datesToPlot", ["Generators", "viewof datesToPlot"], (G, _) => G.input(_));
  main.define("initial thing", function(){return(
0
)});
  main.variable(observer("mutable thing")).define("mutable thing", ["Mutable", "initial thing"], (M, _) => new M(_));
  main.variable(observer("thing")).define("thing", ["mutable thing"], _ => _.generator);
  main.variable(observer("viewof time1")).define("viewof time1", ["datesToPlot","md","Scrubber"], function(datesToPlot,md,Scrubber)
{
  // const button = document.querySelector('[name="b"]');
  // datesToPlot === null ? (button.disabled = true) : (button.disabled = true);

  if (datesToPlot === null) {
    return md`Click the button to animate`;
    //   Scrubber([], {
    //   delay: 250,
    //   autoplay: false,
    //   loop: false,
  ...
  main.variable(observer("mapboxgl")).define("mapboxgl", ["require","html"], async function(require,html)
{
  ...
  main.import("brushFilterX", child5);
  return main;
}
